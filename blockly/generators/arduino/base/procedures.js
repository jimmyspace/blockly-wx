...
Blockly.Arduino['procedures_task'] = function(block) {
  var text_task_name = block.getFieldValue('TASK_NAME')||'""';
  var value_ms = Blockly.Arduino.valueToCode(block, 'MS', Blockly.Arduino.ORDER_ATOMIC) || '0';
  var statements_do = Blockly.Arduino.statementToCode(block, 'DO');
  var func = 'void '+text_task_name+'() {\n'+statements_do+'\n}\n';
  var setup = 'taskTimer_every('+value_ms+', '+text_task_name+');'
  var variable = 'int task_flag = 0';
  var code = '  taskTimer_update {\n    if('+text_task_name+'_flag) {\n      '+text_task_name+'_flag=0;\n      '+text_task_name+'();\n    }  \n}\n'
  //生成函数块
  Blockly.Arduino.userFunctions_[text_task_name] = func;
  //生成setup初始化代码
   Blockly.Arduino.addSetup('userSetupCode',
      setup, true);
