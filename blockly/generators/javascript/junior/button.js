...
//自定义部分
//LED设置某个灯打开/熄灭（根据编号）
Blockly.JavaScript['but_on_flag'] = function(block) {
  var but = block.getFieldValue('BUT');
  var code = 'await ccb.but_on_flag("'+but+'")';
  return [code,Blockly.JavaScript.ORDER_UNARY_POSTFIX];
};

//LED获取某个灯状态
Blockly.JavaScript['but_get_flag'] = function(block) {
 var but_flag = block.getFieldValue('BUT_FLAG');
  var code = 'await ccb.but_get_flag("BUT_F","'+but_flag+'")';
  return [code,Blockly.JavaScript.ORDER_UNARY_POSTFIX];
};
